...
sealed class JobEvent { const JobEvent(); }
final class JobProgress extends JobEvent { const JobProgress(this.value); final double value; }
final class JobFinished extends JobEvent { const JobFinished(this.report); final JobReport report; }
...
abstract final class JobIsolate {
  static Future<JobExecution> start(JobSpec spec) async {
    ...
    final isolate = await Isolate.spawn<(String, SendPort)>(_entry, (spec.encode(), receive.sendPort), debugName: 'AbzarFile conversion');
    execution = JobExecution._(receive, isolate, controller.stream);
    receive.listen((message) {
      if (message is List<Object?> && message.isNotEmpty) {
        switch (message.first) {
          case 'handle': execution.attachHandle(message[1]! as int);
          case 'progress': controller.add(JobProgress(message[1]! as double));
          case 'finished': controller.add(JobFinished(JobReport.fromJson(jsonDecode(message[1]! as String)! as Map<String, Object?>))); controller.close(); execution.close();
          case 'error': controller.addError(NativeException(NativeErrorCode.fromValue(message[1]! as int), message[2]! as String)); controller.close(); execution.close();
        }
      }
    });
    return execution;
  ...
  static void _entry((String, SendPort) message) {
    ...
    calloc.free(json);
    if (job == ffi.nullptr) { message.$2.send(<Object>['error', NativeErrorCode.invalidArgument.value, 'Invalid native job specification.']); return; }
    ...
    try {
      ...
      callback.close();
      final report = bindings.abz_job_report_json(job).cast<Utf8>().toDartString();
      if (result == 0) { message.$2.send(<Object>['progress', 1.0]); message.$2.send(<Object>['finished', report]); }
      else { final parsed = jsonDecode(report)! as Map<String, Object?>; message.$2.send(<Object>['error', result, (parsed['error'] as String?) ?? 'Native conversion failed.']); }
    } finally { bindings.abz_job_destroy(job); }
  }
}
