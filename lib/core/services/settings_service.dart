...
final class AppSettings {
  const AppSettings({this.themeMode = ThemeMode.system, this.locale, this.defaultDpi = 150, this.defaultQuality = 90, this.outputDirectory, this.checkUpdates = true});
...
final class SettingsController extends AsyncNotifier<AppSettings> {
  ...
  Future<AppSettings> build() async {
    ...
    final theme = ThemeMode.values.firstWhere((item) => item.name == prefs.getString('theme'), orElse: () => ThemeMode.system);
  ...
  Future<AppSettings> update(FutureOr<AppSettings> Function(AppSettings) cb, {FutureOr<AppSettings> Function(Object, StackTrace)? onError}) async {
    ...
    if (next.locale == null) { await prefs.remove('locale'); } else { await prefs.setString('locale', next.locale!.languageCode); }
    if (next.outputDirectory == null) { await prefs.remove('outputDirectory'); } else { await prefs.setString('outputDirectory', next.outputDirectory!); }
