...
final formatRegistryProvider =
    FutureProvider<FormatRegistry>((ref) => FormatRegistry.load());
...
final class FormatRegistry {
  ...
  static Future<FormatRegistry> load() async {
    final source =
        await rootBundle.loadString('assets/config/conversion_matrix.json');
    final root = jsonDecode(source)! as Map<String, Object?>;
    return FormatRegistry(
      (root['formats']! as List<Object?>)
          .map((item) => FormatInfo.fromJson(item! as Map<String, Object?>))
          .toList(growable: false),
      (root['conversions']! as List<Object?>)
          .map((item) =>
              ConversionCapability.fromJson(item! as Map<String, Object?>))
          .toList(growable: false),
    );
  }

  List<FormatInfo> targetsFor(String source) {
    final ids = conversions
        .where((item) => item.source == source && item.enabled)
