  ...
  Future<void> exportPdf() async {
    ...
    try {
      ...
      for (final (index, page) in pages.indexed) {
        final extension = p.extension(page.path).replaceFirst('.', '').toLowerCase();
        final corrected = p.join(root.path, 'scan-page-${DateTime.now().microsecondsSinceEpoch}-$index.png');
        await NativeOperations.processScan(inputPath: page.path, outputPath: corrected, format: extension, perspective: perspective, filter: filter, brightness: brightness, contrast: contrast);
        processed.add(corrected);
      }
      final result = p.join(root.path, 'scan-${DateTime.now().millisecondsSinceEpoch}.pdf');
      if (processed.length == 1) {
        await NativeOperations.convert(JobSpec(inputPath: processed.single, outputPath: result, sourceFormat: 'png', targetFormat: 'pdf', dpi: 300));
      } else {
        await NativeOperations.merge(inputPaths: processed, outputPath: result, sourceFormat: 'png', targetFormat: 'pdf');
      }
      for (final path in processed) { final file = File(path); if (await file.exists()) await file.delete(); }
  ...
  Widget build(BuildContext context) {
    ...
    return FeatureScaffold(
      ...
      child: Column(children: <Widget>[
        ...
        Padding(padding: const EdgeInsets.all(16), child: FilledButton.icon(onPressed: pages.isEmpty || running ? null : exportPdf, icon: const Icon(Icons.picture_as_pdf), label: Text(l10n.exportPdf))),
      ]),
    );
  }
}
