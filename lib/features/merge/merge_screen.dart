...
class _MergeScreenState extends State<MergeScreen> {
  ...
  Future<void> pick() async {
    ...
    if (selected.map((entry) => entry.extension).toSet().length > 1 &&
        mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context).sameTypeOnly)));
      return;
    }
    setState(() {
      files = selected;
  ...
  Future<void> merge() async {
    ...
    try {
      ...
      final path = p.join(directory.path,
          '${p.basenameWithoutExtension(files.first.path)}-merged.$format');
      await NativeOperations.merge(
          inputPaths: files.map((entry) => entry.path).toList(growable: false),
          outputPath: path,
          sourceFormat: format,
          targetFormat: format);
        ...
        children: <Widget>[
          ...
          Padding(
              ...
              child: FilledButton.icon(
                  onPressed: files.length < 2 || running ? null : merge,
