...
class _PptxEditorScreenState extends State<PptxEditorScreen> {
  ...
  Future<void> open() async {
    ...
    try {
      ...
      for (final line
        ...
        slides.add(TextEditingController(text: line));
      }
  ...
  Future<void> save() async {
    final output = path ??
        await FilePicker.platform.saveFile(
            ...
            allowedExtensions: const <String>['pptx']);
    ...
    try {
      ...
      for (final (index, slide) in slides.indexed) {
        final text = File(p.join(root.path,
            'slide-${DateTime.now().microsecondsSinceEpoch}-$index.txt'));
      ...
      if (parts.length == 1) {
        await File(parts.single).copy(output);
        await File(parts.single).delete();
      } else {
  ...
  void preview() => showDialog<void>(
      ...
      builder: (context) => Dialog.fullscreen(
          child: Scaffold(
              appBar: AppBar(
                  title:
                      Text(AppLocalizations.of(context).presentationPreview)),
              body: Center(
                  child: Text(slides[selected].text,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.pptxEditor), actions: <Widget>[
        IconButton(
            tooltip: l10n.presentationPreview,
              ...
              child: ReorderableListView(
                  onReorder: (oldIndex, newIndex) {
                    setState(() {
                      ...
                      slides.insert(newIndex, slides.removeAt(oldIndex));
                      selected = newIndex;
                    });
                  },
                  children: slides.indexed
                      .map((item) => Card(
                          key: ValueKey(item.$2),
                          child: ListTile(
                              selected: selected == item.$1,
                              onTap: () => setState(() => selected = item.$1),
                              title: Text('${l10n.pages} ${item.$1 + 1}'))))
                      .toList())),
                ...
                Wrap(
                  children: <Widget>[
                    IconButton(
                        ...
                        onPressed: () => setState(
                            () => slides.add(TextEditingController()))),
                    IconButton(
                    ...
                    IconButton(
                        ...
                        onPressed: () => slides[selected].text += '\n• '),
                    IconButton(
                        tooltip: l10n.delete,
                        ...
                        onPressed: slides.length == 1
                            ? null
                            : () => setState(() {
                                  slides.removeAt(selected).dispose();
                                  selected = 0;
                                })),
                  ],
                ),
