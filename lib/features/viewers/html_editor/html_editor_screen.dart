...
class _HtmlEditorScreenState extends State<HtmlEditorScreen> {
  ...
  Future<void> open() async {
    ...
    if (files.isNotEmpty) {
      source.text = await File(files.first.path).readAsString();
  ...
  Future<void> save() async {
    ...
    if (output != null) {
      await File(output).writeAsString(source.text, flush: true);
      path = output;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.htmlEditor), actions: <Widget>[
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          final sourcePane = Column(children: <Widget>[
            ListTile(title: Text(l10n.source)),
            Expanded(
                child: TextField(
                    controller: source,
          ...
          final previewPane = Column(children: <Widget>[
            ListTile(title: Text(l10n.preview)),
            Expanded(
                child: SingleChildScrollView(
                    padding: const EdgeInsets.all(24),
                    child: SelectableText(plain(source.text),
