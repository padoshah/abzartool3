...
final class UpdateService {
  ...
  Future<AppRelease?> check() async {
    ...
    if (response.statusCode != 200)
      throw HttpException('GitHub Releases returned ${response.statusCode}.');
    final release = AppRelease.fromJson(response.body.isEmpty
        ? <String, Object?>{}
        : jsonDecode(response.body)! as Map<String, Object?>);
    final installed = (await PackageInfo.fromPlatform()).version;
    final skipped = (await SharedPreferences.getInstance())
        .getString('skippedUpdateVersion');
    return _newer(release.version, installed) && skipped != release.version
        ? release
        : null;
  }
  ...
  Future<File> downloadAndVerify(
      AppRelease release, void Function(double) progress) async {
    final suffix =
        Platform.isAndroid ? 'android-universal.apk' : 'windows-x64-setup.exe';
    final expectedName = 'AbzarFile-v${release.version}-$suffix';
    final asset =
        release.assets.where((item) => item.name == expectedName).first;
    final checksumName =
        Platform.isAndroid ? 'checksums-android.txt' : 'checksums-windows.txt';
    final checksumAsset =
        release.assets.where((item) => item.name == checksumName).first;
    final checksumResponse = await http
        .get(Uri.parse(checksumAsset.url))
        .timeout(const Duration(seconds: 20));
    if (checksumResponse.statusCode != 200)
      throw const HttpException('Unable to download release checksums.');
    final expected =
        RegExp('([a-fA-F0-9]{64})\\s+\\*?${RegExp.escape(expectedName)}')
            .firstMatch(checksumResponse.body)
            ?.group(1)
            ?.toLowerCase();
    if (expected == null)
      throw const FormatException(
          'The release checksum file has no matching entry.');
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (await sha256File(partial) != expected) {
          await partial.delete();
          throw const FormatException(
              'The downloaded update checksum does not match.');
        }
  ...
  Future<void> _downloadResumable(
      ReleaseAsset asset, File partial, void Function(double) progress) async {
    ...
    try {
      await for (final bytes in response.stream) {
        ...
        progress(asset.size == 0 ? 0 : received / asset.size);
      }
    } finally {
      await sink.close();
    }
    if (asset.size > 0 && received != asset.size)
      throw const HttpException('The update download is incomplete.');
  }
}
