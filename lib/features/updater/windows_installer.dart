...
abstract final class WindowsInstaller {
  static Future<void> verifyAndInstall(File setup) async {
    ...
    final script = "\$candidate=Get-AuthenticodeSignature -LiteralPath '$setupPath'; "
    ...
    final verify = await Process.run('powershell.exe', <String>['-NoProfile', '-NonInteractive', '-Command', script]);
    if (verify.exitCode != 0 || (verify.stdout as String).trim().isEmpty) {
      throw const FileSystemException('The update signature is invalid or has a different publisher.');
    }
    await Process.start(setup.path, const <String>['/SILENT', '/CLOSEAPPLICATIONS', '/RESTARTAPPLICATIONS', '/NORESTART'], mode: ProcessStartMode.detached);
    exit(0);
  }
}
