...
final convertControllerProvider =
    StateNotifierProvider<ConvertController, ConvertState>(
        (ref) => ConvertController(ref));
...
final class ConvertController extends StateNotifier<ConvertState> {
  ...
  Future<void> run() async {
    for (var index = 0; index < state.items.length; index++) {
      ...
      final output = await _storage.reserveOutput(file.path, state.targetFormat,
          preferredDirectory: settings?.outputDirectory);
      _replace(
          index,
      ...
      try {
        ...
        await for (final event in execution.events) {
          if (event is JobProgress)
            _replace(index, state.items[index].copyWith(progress: event.value));
        ...
        final cancelled =
            error is NativeException && error.code == NativeErrorCode.cancelled;
        _replace(
            index,
            state.items[index].copyWith(
                status: cancelled ? QueueStatus.cancelled : QueueStatus.failed,
                error: error));
      } finally {
  ...
  void cancel(int index) {
    _executions[index]?.cancel();
    _replace(index, state.items[index].copyWith(status: QueueStatus.cancelled));
  }

  void retry(int index) => _replace(index,
      state.items[index].copyWith(status: QueueStatus.waiting, progress: 0));
  void _replace(int index, QueueItem item) {
